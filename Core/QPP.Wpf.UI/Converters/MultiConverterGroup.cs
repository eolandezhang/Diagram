        ...
        public object[] ConvertBack(object value, Type[] targetTypes, object parameter, CultureInfo culture)
        {
            ...
            for (var i = this.Steps.Count - 1; i >= 0; --i)
            {
                ...
                if (step.Converters.Count != stepValues.Length)
                    throw new InvalidOperationException("Step with index {0} produced {1} values but the subsequent step (index {2}) requires {3} values.".FormatArgs(i + 1, stepValues.Length, i, step.Converters.Count));
                //exceptionHelper.ResolveAndThrowIf(step.Converters.Count == 0, "EachStepMustHaveAtLeastOneConverter");
