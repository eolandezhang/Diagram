...
{
    using QPP.Wpf.UI.Converters.Expressions.Nodes;
    using System;
    ...
    internal sealed class Parser : IDisposable
    {
        ...
        public Node ParseExpression()
        {
            ...
            {
                throw new ParseException("Unexpected input '{0}'.".FormatArgs(this.currentToken.Value));
        ...
        private CastNode ParseCast()
        {
            ...
            {
                throw new ParseException("Cannot cast to type '{0}'.".FormatArgs(targetTypeStr));
        ...
        private Node ParseHexadecimalConstant()
        {
            ...
            if (numberStr.Length == 0)
                throw new ParseException("Invalid number: {0}".FormatArgs(this.currentToken.Value));
        ...
        private Node ParseIntegral()
        {
            ...
            {
                long val;
                if (!long.TryParse(numberStr, out val))
                    throw new ParseException("Invalid number: {0}".FormatArgs(this.currentToken.Value));
        ...
        {
            int val;
            if(!int.TryParse(this.currentToken.Value, out val))
                throw new ParseException("Invalid number: {0}".FormatArgs(this.currentToken.Value));
        ...
        private Node ParseReal()
        {
            ...
            switch (realType)
            {
                ...
                case RealType.Single:
                    {
                        float val;
                        if (!float.TryParse(numberStr, out val))
                            throw new ParseException("Invalid number: {0}".FormatArgs(this.currentToken.Value));
                ...
                case RealType.Decimal:
                    {
                        decimal val;
                        if (!decimal.TryParse(numberStr, out val))
                            throw new ParseException("Invalid number: {0}".FormatArgs(this.currentToken.Value));
        ...
        {
            this.EnsureMoreTokens();
            if (!this.currentToken.Equals(type, value))
                throw new ParseException("Unexpected input '{0}'. Expected '{1}'.".FormatArgs(this.currentToken.Value, value));
